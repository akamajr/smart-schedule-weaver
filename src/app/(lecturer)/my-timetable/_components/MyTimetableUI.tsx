    ...
    <div className="space-y-6">
      ...
      {activeTimetable && (
        <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm font-medium text-muted-foreground">
            {activeTimetable.academic_year && <span>{activeTimetable.academic_year}</span>}
            {activeTimetable.semester && <span>•</span>}
            {activeTimetable.semester && <span>{activeTimetable.semester} Semester</span>}
            {activeTimetable.type && <span>•</span>}
            {activeTimetable.type && <span>{activeTimetable.type}</span>}
            {activeTimetable.level && <span>•</span>}
            {activeTimetable.level && <span>Level {activeTimetable.level}</span>}
          </div>
        </div>
      )}
      ...
      <div className="overflow-x-auto rounded-3xl border border-border bg-card p-4 shadow-card print:p-0 print:border-none print:shadow-none">
        {(viewMode === "personal" && personalLoading) || (viewMode === "department" && deptLoading) ? (
          ...
          <div className="h-64 flex flex-col items-center justify-center text-muted-foreground">
            <CalendarDays className="h-10 w-10 mb-3 opacity-20" />
            <p>No published timetable found for you.</p>
          </div>
