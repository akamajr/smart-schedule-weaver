...
const AIScheduler = () => {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="AI Scheduler"
        description="Smart suggestions, auto-resolution, and optimized room allocation."
        actions={
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2 flex h-[600px] flex-col rounded-2xl border border-border bg-card shadow-soft">
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex gap-3 ${m.role === "user" ? "flex-row-reverse" : ""} animate-fade-in`}>
                <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${
                  m.role === "ai" ? "gradient-primary text-primary-foreground" : "bg-secondary text-foreground"
                }`}>
          ...
          <div className="border-t border-border p-4">
            ...
            <div className="flex gap-2">
              ...
              />
              <Button onClick={() => send()} className="h-11 rounded-xl gradient-primary text-primary-foreground">
